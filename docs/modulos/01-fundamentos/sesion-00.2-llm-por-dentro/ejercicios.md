...
Abrí el [**OpenAI Tokenizer**](https://platform.openai.com/tokenizer) (es público, sin login). Vas a tokenizar 5 strings y completar la tabla.

**Tarea:** copia y pega cada string, anota cuántos tokens tiene, y escribí en una frase qué te llamó la atención.
...
**Pregunta de cierre:** ¿qué string tiene la peor relación palabras-por-token? ¿Por qué creés que pasa?
...
**Sin probar nada todavía**, escribí tu predicción de qué va a pasar para cada caso:

Dado el prompt: `"Dame 3 nombres para una cafetería temática de ciencia ficción."`

- Con `temperature = 0`, ¿qué pasa si lo corrés 5 veces?
- Con `temperature = 0.7`, ¿qué pasa si lo corrés 5 veces?
- Con `temperature = 1.5`, ¿qué pasa si lo corrés 5 veces?

Una vez que tengas tus predicciones escritas, **anotalas y conservalas**. En S01.2 vamos a hacer este experimento real con la primera llamada al LLM, y vas a poder contrastar.
...
Tenés un asistente que usa Claude Sonnet 4.6 (context window: 1M tokens). Tu setup actual:
...
2. Si en lugar de Sonnet 4.6 usás Haiku 4.5 (200K context), ¿seguís entrando? ¿Cuánto te queda?
3. Si el costo del modelo principal sube y querés reducir input, ¿qué dos optimizaciones de las vistas en S00.1 podrías aplicar PRIMERO antes de cambiar de modelo?

> **Soluciones:**
> 1. Total usado = 5 + 50 + 8 + 0.5 + 1 = **64.5K**. Libre = 1M - 64.5K = **~935K tokens**.
> 2. En Haiku: total 64.5K, libre 200K - 64.5K = **~135K**. Sí entrás cómodo.
...
Para cada uno de los siguientes mensajes que llegarían al asistente de TiendaPro, decidí qué `temperature` usarías y por qué (1 frase de justificación):
...
> 2. → `0.7-0.8` (querés variedad y creatividad en las sugerencias).
...
Elegí UN producto LLM que uses regularmente (ChatGPT, Claude.ai, Cursor, Perplexity, Notion AI, etc.) y observalo durante una sesión real (5-10 minutos). Anotá:

1. **¿Hay streaming?** ¿Empieza a mostrar texto en <500ms o tenés que esperar la respuesta completa?
2. **Latencia hasta el primer token vs. latencia total.** ¿Sentís diferencia entre "rápido en empezar" y "rápido en terminar"?
3. **Adivina la temperature.** Si pedís lo mismo dos veces, ¿te da respuestas idénticas o variadas? ¿Qué te dice eso?
...
Hacé un mini-informe de 5 puntos (uno por pregunta). El objetivo: entrenar el ojo para leer las decisiones de diseño que hay detrás de un producto LLM.

---

## 4. Aporte al proyecto integrador

TiendaPro sigue sin código (S01.1 arranca eso). Pero ya podés cerrar 3 decisiones de diseño que vas a aplicar después:
...
Anotá tus respuestas en el mismo doc local que arrancaste en S00.1. En S01.2 vamos a comparar tus respuestas con lo que terminamos implementando.
