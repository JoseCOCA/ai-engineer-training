/**
 * Chat service — wrapper sobre Vercel AI SDK con:
 *   - defaults de producto
 *   - retry con backoff (errores transitorios)
 *   - fallback al proveedor secundario si el primario falla
 *   - instrumentación: latencia, tokens, costo, flow
 *   - logging append-only a logs/calls.jsonl
 *   - streaming con cancelación y manejo de errores
 *
 * Esta es la frontera arquitectural del producto: el resto de la app
 * NUNCA importa `ai` ni los SDKs de proveedores. Importa solo `chat`
 * y `chatStream` desde acá.
 */
  ...
  try {
    const result = await withRetry(() => callProvider(PRIMARY_PROVIDER, req), {
      onRetry: (attempt, error) => {
        console.warn(
          `[chat][${flow}] retry ${attempt} after error: ${
 ...
 * Streaming wrapper.
 *
 * Devuelve un async iterable de chunks de texto + una promesa
 * `finished` que resuelve con la metadata completa al terminar.
 *
 * Maneja errores mid-stream sin romper el iterable: emite un mensaje
 * de error como último chunk y termina limpio.
 */
